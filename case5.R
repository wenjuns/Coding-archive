...
N=1200
q=450                # number of IVs
p = 20               # number of exogeneous variables
sparse = 0.1*q 
...
sigmaXosqfactor = 1
cs_Xo = matrix(0, p,p)
cor_Xo = sample(seq(0, rho_Xo, by = 0.05), size = ((p^2-p)/2), replace=TRUE)
cs_Xo[lower.tri(cs_Xo)]=cor_Xo
tcs_Xo=t(cs_Xo)
cs_Xo[upper.tri(cs_Xo)]=tcs_Xo[upper.tri(tcs_Xo)]
diag(cs_Xo)=1
sigma_Xo = sigmaXosqfactor*cs_Xo
sigma_Xo = make.positive.definite(sigma_Xo)
#rho_Xo = 0.2
#sigmaXosqfactor = 1
...
{ 
  ...
  fn_MA = function(y, xe, xI, xo, n, t, K)  {
    ...
    bias_beta=a_bias*t(u)%*%solve(psi, tol=1e-30)%*%Ik_vector*(t(u)%*%solve(psi, tol=1e-30)%*%Ik_vector-1)/(t(u)%*%solve(psi, tol=1e-30)%*%u-a_bias*(t(u)%*%solve(psi, tol=1e-30)%*%Ik_vector)^2)* be
...
fcn_one <- function(n,al_o) {
  ...
  bo = sample(c(seq(-5,-1,by=1),seq(1,5,by=1)),size=p,replace=TRUE)  
